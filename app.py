...
# ----------------------------------------------------------------------
CAL_SLOPE = 0.7  # spread of the calibrated confidence per order of magnitude

def calibrate(raw_fake, threshold, slope=CAL_SLOPE):
    """Map the raw (squashed) deepfake score to an interpretable confidence
    centred on the decision threshold:
      raw == threshold -> 0.5 ;  raw >> threshold -> ~1 ;  raw << threshold -> ~0.
    Monotonic, so the decision (calibrated >= 0.5) is identical to raw >= threshold."""
    z = np.log10(max(raw_fake, 1e-30)) - np.log10(max(threshold, 1e-30))
    return float(1.0 / (1.0 + np.exp(-slope * z)))
...
def predict(model, audio, threshold):
    ...
    raw_fake = float(fake.max())
    conf_fake = calibrate(raw_fake, threshold)   # interpretable, threshold-centred
    return {
        "fake_prob": conf_fake,            # calibrated -> shown in the UI
        "real_prob": 1.0 - conf_fake,
        "raw_fake": raw_fake,              # original score (for transparency)
        "is_fake": conf_fake >= 0.5,       # identical decision to raw_fake >= threshold
...
if uploaded_file:
    ...
    try:
        ...
        if audio.size < SR // 2:  # < 0.5 s of usable audio
            ...
            if res["is_fake"]:
                st.error(f"🚨 Result: Deepfake Detected  ({fmt_prob(res['fake_prob'])} confidence)")
            else:
                st.success(f"✅ Result: Genuine Audio  ({fmt_prob(res['real_prob'])} confidence)")

            st.caption(
                f"Analysed {res['n_windows']} window(s) across the whole clip "
                f"(flagged if any window looks synthetic). "
                f"Confidence is calibrated around the model's decision boundary; "
                f"raw deepfake score = {res['raw_fake']:.2e}."
            )
            if res["n_windows"] > 1:
