...
if uploaded_file:
    ...
    try:
        ...
        if audio.size < SR // 2:  # < 0.5 s of usable audio
            ...
            col1.metric("Genuine (Human)", f"{res['real_prob']*100:.1f}%")
            col2.metric("Deepfake (AI)",   f"{res['fake_prob']*100:.1f}%")

            if res["is_fake"]:
                st.error(f"🚨 Result: Deepfake Detected  (decision threshold {threshold:.2f})")
            else:
                st.success(f"✅ Result: Genuine Audio  (decision threshold {threshold:.2f})")
